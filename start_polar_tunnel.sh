#!/usr/bin/env bash
# Start the local Polar MCP server and its private OpenAI Secure MCP Tunnel.
set -euo pipefail
...
for _ in {1..30}; do
  ...
  if ! kill -0 "$polar_pid" 2>/dev/null; then
    echo "Polar MCP server stopped before the tunnel could start." >&2
    exit 1
  fi
  sleep 0.2
done
...
if ! curl --silent --output /dev/null --max-time 1 http://127.0.0.1:8000/mcp; then
  echo "Polar MCP server did not become reachable on localhost:8000." >&2
  exit 1
fi
